...
export class ImagesProbe extends DurableObject<Env> {
  async processFetchedDraw(): Promise<{ bytes: number; steps: string[] }> {
    ...
    const watermarkedResponse = await this.env.IMAGES.input(arrayBufferToReadableStream(testImage))
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    const image = this.env.IMAGES.input(arrayBufferToReadableStream(testImage));
    const result =
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (url.pathname === "/images-transform") {
      const result = await env.IMAGES.input(arrayBufferToReadableStream(testImage))
    ...
    if (url.pathname === "/images-draw-inline") {
      const result = await env.IMAGES.input(arrayBufferToReadableStream(testImage))
        .draw(arrayBufferToReadableStream(testImage), {
          bottom: 0,
          opacity: 0,
          right: 0,
        })
